...
export default {
    ...
    methods: {
        ...
        show(index) {
            this.$Modal.info({
                title: "书籍信息",
                content: `书名：${this.data6[index].title}<br>书籍编号：${
                    this.data6[index].number
                }<br>借阅者学号：${this.data6[index].raccount}<br>借阅者姓名：${
                    this.data6[index].rname
                }<br>借阅时间：${this.data6[index].time}<br>应归还时间：${
                    this.data6[index].backtime
                }<br>状态：${this.data6[index].condi}`
            });
        },
        reback(index) {
            ...
            this.$http
                .post(
                    that.GLOBAL.serverPath + "/excise/reback",
                    {
        ...
        request(currentPage) {
            ...
            this.$http
                .post(
                    that.GLOBAL.serverPath + "/excise/getAllBorrowRecords",
                    {
                ...
                .then(function(res) {
                    ...
                    that.data7.forEach(e => {
                        ...
                        if (time < e.inttime) {
                            ...
                            var delayDay = Math.ceil(
                                (time - e.inttime) / 86400000
                            );
