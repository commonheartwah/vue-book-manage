        ...
        >
            ...
            <Row>
                ...
                <Col :xs="8" :sm="8" :md="8" :lg="8">
                    <h2 id="verifyCode" @click="createCode()">
                        {{ verifyCode }}
                    </h2>
                </Col>
            </Row>
            ...
            this.$refs[name].validate(valid => {
                if (valid) {
                    if (that.verifyCode === that.formItem.code) {
                        that.$http
                            .post(
                                that.GLOBAL.serverPath + "/excise/login",
                                {
                            ...
                            .then(function(res) {
                                ...
                                if (res.data.result === "yes") {
                                    ...
                                    window.localStorage.setItem(
                                        "userId",
                                        res.data.loginUser.rid
                                    );
                                    window.localStorage.setItem(
                                        "account",
                                        res.data.loginUser.account
                                    );
                                    window.localStorage.setItem(
                                        "username",
                                    ...
                                    window.localStorage.setItem(
                                        "sex",
                                        res.data.loginUser.sex
                                    );
                                    window.localStorage.setItem(
                                        "condi",
                                        res.data.loginUser.condi
                                    );
