    ...
    methods: {
        ...
        show(index) {
            this.$Modal.info({
                ...
                content: `书名：${this.data6[index].title}<br>作者：${
                    this.data6[index].author
                }<br>出版社：${this.data6[index].publisher}<br>出版时间：${
                    this.data6[index].publishtime
                }<br>副本数量：${this.data6[index].num}<br>可借数量：${
                    this.data6[index].count
                }<br>可借副本编号：<span style="color:red;">${
                    this.data6[index].suba
                }</span><br>介绍：${this.data6[index].descri}`
            });
        },
        remove(index) {
        ...
        request(currentPage) {
            ...
            this.$http
                .post(
                    that.GLOBAL.serverPath + "/excise/getAllAlbums",
                    {
        ...
        ok(name) {
            ...
            this.$refs[name].validate(valid => {
                if (valid) {
                    that.$http
                        .post(
                            that.GLOBAL.serverPath + "/excise/addAlbum",
                            {
        ...
        ok2(name) {
            ...
            this.$refs[name].validate(valid => {
                if (valid) {
                    that.$http
                        .post(
                            that.GLOBAL.serverPath + "/excise/addSubAlbum",
                            {
                                aid: that.currIndex,
